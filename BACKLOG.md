...
- [x] Вынести OrderManager в очередь/воркеры, добавить таймауты/ретраи и resilient API wrapper (см. Б4.1/Б4.2).

**В работе:**
- [ ] Реализовать alerting и мониторинг деградации API (DNS/ключи) с уведомлениями и runbook (Б4.2.2).
- [ ] Привести `range_trading_strategy` и остальные стратегии к модульной логике (индикаторы → сигнал → позиционирование) и выровнять параметры (минимальный размер, фильтры).
- [ ] Добавить dry-run режим (`DRY_RUN=true`) с моками API и фиктивными fill’ами для безопасных тестов.
- [ ] Упростить нейро-интеграцию до уровня MVP: строгий tz-aware анализ, очистка кэшей и backfill данных перед подключением ML.

**Очередь следующая:**
- [ ] Запустить CI-пайплайн: unit/contract tests, mocked Bybit, dry-run симуляции.
...
**Next action:** Зафиксировать алертинг по сети (Б4.2) и подготовить тестовый контур (dry-run/CI), затем перейти к observability.
...
- 🧪 Тестирование/CI: отсутствуют автоматические тесты и dry-run — критичный пробел.
